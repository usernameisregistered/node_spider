...
if (args.length != 5) {
    ...
    type = args[4] && args[4] == 1 ? "nn" : 'nt';
    fs.stat(path.join(__dirname, "nn"), (err, stat) => {
        if (!stat || !stat.isDirectory()) {
            console.log("创建必要的文件夹nn")
            fs.mkdirSync(path.join(__dirname, "nn"))
        }
        fs.stat(path.join(__dirname, "nt"), (err, stat) => {
            if (!stat || !stat.isDirectory()) {
                console.log("创建必要的文件夹nt")
                fs.mkdirSync(path.join(__dirname, "nt"))
            }
            getHtml();
        })
    })
...
function getInfo($) {
    ...
    if (size == 0) {
        ...
        for (let i = 1; i < size; i++) {
            ...
            let tempObj = {};
            tempObj.ip = el.eq(1).text()
            tempObj.port = el.eq(2).text()
            tempObj.cryptonym = el.eq(4).text()
            tempObj.protocol = el.eq(5).text()
            tempObj.position = el.eq(3).text()
